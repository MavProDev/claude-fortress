...
For non-AI findings, emit `ATLAS: skipped (non-AI finding)` per the Step 4.2 Output Format — do not compute a technique mapping. The line must always be present so downstream consumers see a stable schema.
...
**Activation gate:** Run this step only if the finding originated from Squad 14 (AI/LLM), Squad 15 (Single-Agent & MCP Exploitation), Squad 23 (Multi-Agent, Agentic Infrastructure & NHI Security), or Squad 25 (Computer-Use Security), OR the finding involves tool-calling, MCP servers, autonomous loops, A2A protocol, or multi-agent orchestration. For all other findings, emit `AIVSS: skipped (non-agentic finding)` per the Step 4.2 Output Format — do not compute the score. The line must always be present so downstream consumers see a stable schema.
...
For non-agentic findings, emit `AIVSS: skipped (non-agentic finding)` per the Step 4.2 Output Format — never omit the line entirely.
